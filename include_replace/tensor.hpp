    ...
    internal::float_type low = 0.0f, internal::float_type high = 1.0f)
{
    ...
    {   return static_cast<internal::float_type>(b);
...
// Data must be stored row-wise (and channels_last).
// No scales.
inline tensor tensor_from_bytes_no_scale(const std::uint8_t* value_ptr,
    std::size_t height, std::size_t width, std::size_t channels,
    internal::float_type low = 0.0f, internal::float_type high = 1.0f)
{
    const std::vector<std::uint8_t> bytes(
        value_ptr, value_ptr + height * width * channels);
    auto values = fplus::transform_convert_channels<float_vec>(
    [mean_val = B_MEAN](std::uint8_t b) -> internal::float_type {return static_cast<internal::float_type>(b) - mean_val;},
    [mean_val = G_MEAN](std::uint8_t b) -> internal::float_type {return static_cast<internal::float_type>(b) - mean_val;},
    [mean_val = R_MEAN](std::uint8_t b) -> internal::float_type {return static_cast<internal::float_type>(b) - mean_val;},
     bytes);
    return tensor(tensor_shape(height, width, channels), std::move(values));
}
