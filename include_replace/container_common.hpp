...
#include <iostream>
#define B_MEAN 103.939
#define G_MEAN 116.779
...
// transform_convert of different channels
template <typename ContainerOut, typename Fb, typename Fg, typename Fr, typename ContainerIn>
ContainerOut transform_convert_channels(Fb b, Fg g, Fr r, const ContainerIn& xs)
{
    internal::trigger_static_asserts<internal::unary_function_tag, Fb, typename ContainerIn::value_type>();
    internal::trigger_static_asserts<internal::unary_function_tag, Fg, typename ContainerIn::value_type>();
    internal::trigger_static_asserts<internal::unary_function_tag, Fr, typename ContainerIn::value_type>();
    ContainerOut ys;
    auto container_size = size_of_cont(xs);
    internal::prepare_container(ys, container_size);
    auto xs_start = std::begin(xs);
    auto ys_start = internal::get_back_inserter<ContainerOut>(ys);
    while(xs_start != std::end(xs)) {
        ys_start = std::transform(xs_start, xs_start + 1, ys_start, b);
        std::advance(xs_start,1);
        ys_start = std::transform(xs_start, xs_start + 1, ys_start, g);
        std::advance(xs_start,1);
        ys_start = std::transform(xs_start, xs_start + 1, ys_start, r);
        std::advance(xs_start,1);
    }
    /*
    auto it = internal::get_back_inserter<ContainerOut>(ys);
    it = std::transform(std::begin(xs), std::begin(xs) + container_size / 3, it, b);
    it = std::transform(std::begin(xs) + container_size / 3, std::begin(xs) + 2 * container_size / 3, it, g);
    std::transform(std::begin(xs) + 2 * container_size / 3, std::end(xs), it, r);
    */
    return ys;
}
